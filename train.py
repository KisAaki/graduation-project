...
def train(net, device, q, log_txt_file, val_datapath, models_train_best, models_train_last, lr=1e-4, lr_de_epoch=25000,
    ...
    for epoch in range(1, epochs+1):
        # img: [40,3,224,224] cls_gt: [8,78]  mask_gt: [40,3,224,224]
        img, cls_gt, mask_gt = q.get()  
        net.zero_grad()
        img, cls_gt, mask_gt = img.to(device), cls_gt.to(device), mask_gt.to(device)
        # print('cls_gt',cls_gt)
        # print('mask_gt', mask_gt)
        # img_data = np.reshape(mask_gt.cpu().numpy(), (40, 224, 224, 1))
        # plt.imshow(img_data[0])
        # plt.show()
        # path='./save/image'+str(epoch)+'.png'
        # plt.savefig(path)

        # 预测结果：种类，掩码
        pred_cls, pred_mask = net(img)
        # print('pred_cls.size()', pred_cls.size())  # [8,78]
        # print('pred_mask.size()', pred_mask.size())  # [40,224,224]
